...
export default function PlacementPage() {
  ...
  const crumbs = [{ label: "Learn", href: "/learn" }, { label: "Placement" }];
      ...
      <main className="shell">
        ...
        <section className="card">
          ...
          <ul className="testMeta">
            ...
            <li>Total beginner? Start at <Link href="/learn/lesson/0.1">Lesson 0·1</Link> instead.</li>
          </ul>
          <div className="stepNav">
            <button type="button" className="primary" onClick={() => { setStartedAt(Date.now()); setPhase("running"); }}>Start</button>
            <Link href="/learn" className="secondary buttonLike">Not now</Link>
          </div>
        </section>
      </main>
    );
  ...
  if (phase === "running" && block) {
    return (
      <main className="shell lessonShell">
        <Crumbs items={[{ label: "Learn", href: "/learn" }, { label: "Placement" }, { label: `Unit ${block.unit} of ${blocks.length}` }]} />
  ...
  return (
    <main className="shell">
      ...
      <section className="card">
        ...
        <div className="stepNav">
          <button type="button" className="primary" onClick={() => router.push(target ? `/learn/lesson/${target}` : "/learn")}>{target ? `Open Lesson ${target.replace(".", "·")}` : "Back to the course"}</button>
          <Link href="/learn" className="secondary buttonLike">Course map</Link>
        </div>
      </section>
    </main>
  );
