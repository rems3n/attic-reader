...
export default function TrackPage() {
  ...
  const baseCrumbs = [{ label: "Learn", href: "/learn" }, { label: "Tracks", href: "/learn#tracks" }];
  if (error) return <PageError message={error} crumbs={baseCrumbs} back={{ href: "/learn#tracks", label: "All tracks" }} />;
    ...
    <main className="shell trackPage">
      ...
      <section className="hero">
        ...
        <div className="actions">
          ...
          {ts.next && <Link href={`/learn/lesson/${ts.next}`} className="primary buttonLike">{cp.lessons[ts.next]?.status === "in-progress" ? "Resume" : "Open"} lesson {ts.next.split(".")[1]} →</Link>}
        </div>
      ...
      <ol className="trackLadder">
        {track.lessons.map((l, i) => {
          ...
          return <li key={l.id} className={`ladderItem ${open ? "open" : "locked"}`}>{open ? <Link href={`/learn/lesson/${l.id}`}>{body}<span className="tileGo" aria-hidden="true">›</span></Link> : <div>{body}</div>}</li>;
        })}
        <li className={`ladderItem gate ${gate.state === "open" || gate.state === "passed" ? "open" : "locked"}`}>
          {gate.state === "open" || gate.state === "passed" ? (
            <Link href={`/learn/test/${track.gate}`}>
      ...
      <section className="card">
        ...
        {track.words.length > 0 && (
          <p><Link href={`/words?words=${track.words.map((w) => w.id).join(",")}&from=${encodeURIComponent(`${track.title_en} track`)}`} className="secondary buttonLike">Study the track words</Link></p>
        )}
        {showWords && (
          <ul className="trackWords" id="track-words">
            {track.words.map((w) => (
              <li key={w.id}>
                ...
                <Link href={`/words/${w.id}`} lang="grc" className="trackWordLemma">{w.headword}</Link>
