...
export default function LessonPage() {
  ...
  if (error) return <PageError message={error} crumbs={[{ label: "Learn", href: "/learn" }, { label: `Lesson ${id}` }]} back={{ href: "/learn", label: "Back to the course" }} />;
  if (!lesson) return <PageLoading label={`Loading lesson ${id}…`} crumbs={[{ label: "Learn", href: "/learn" }, { label: `Lesson ${id}` }]} />;
    ...
    <main className="shell lessonShell">
      <header className="lessonHead">
        <div className="lessonCrumbs">
          <Crumbs
            items={
              lesson.track
                ? [{ label: "Learn", href: "/learn" }, { label: "Tracks", href: "/learn#tracks" }, { label: lesson.track.title_grc, href: `/learn/track/${lesson.track.id}`, lang: "grc" }, { label: `Lesson ${lesson.position.in_unit} of ${lesson.position.unit_size}` }]
                : [{ label: "Learn", href: "/learn" }, { label: `Unit ${lesson.unit.n}`, href: `/learn#unit-${lesson.unit.n}` }, { label: `Lesson ${lesson.id}` }]
            }
          />
      ...
      {current.id === "vocab" && (
        <section className="card">
            ...
            {lesson.vocab.map((v) => (
              <li key={v.id} className="vocabTile">
                ...
                <div className="vocabText">
                  <Link href={`/words/${encodeURIComponent(v.id)}`} lang="grc" className="vocabLemma">{v.headword}</Link>
          ...
          <div className="stepNav">
            <Link href="/words" className="secondary buttonLike">Study cards</Link>
          ...
          {quizResult ? (
            <div className="stepDone">
              ...
              <div className="stepNav">
                ...
                {quizResult.score >= PASS && lesson.position.next && <Link href={`/learn/lesson/${lesson.position.next}`} className="primary buttonLike">Next lesson →</Link>}
                {quizResult.score >= PASS && !lesson.position.next && <Link href={lesson.track ? `/learn/track/${lesson.track.id}` : "/learn"} className="primary buttonLike">{lesson.track ? "Back to the track" : "Back to the course"}</Link>}
                {quizResult.score >= PASS && lesson.unit.test && <Link href="/learn" className="secondary buttonLike">Course home</Link>}
              </div>
            </div>
          ) : (
            <>
      ...
      <nav className="lessonFooter" aria-label="Lesson navigation">
        ...
        <span className="lessonJump small">
          {lesson.position.prev && <Link href={`/learn/lesson/${lesson.position.prev}`} aria-label={`Previous lesson, ${lesson.position.prev}`}>‹ {lesson.position.prev}</Link>}
          {lesson.position.next && <Link href={`/learn/lesson/${lesson.position.next}`} aria-label={`Next lesson, ${lesson.position.next}`}>{lesson.position.next} ›</Link>}
        </span>
