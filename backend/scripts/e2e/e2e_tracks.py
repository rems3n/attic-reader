...
- /course/track/history: the ladder, "make this my track", track words link
...
- Reader: a library passage shows its known-word percentage; the coverage
  panel under the text links to a deck of the new words on /vocab
"""
...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        page.goto(f"{FRONT}/course/lesson/hist.1?step=6")
        ...
        page.goto(f"{FRONT}/course/track/history")
        expect(page.locator(".ladderItem.gate a")).to_be_visible()
        page.goto(f"{FRONT}/course/test/gate-hist")
